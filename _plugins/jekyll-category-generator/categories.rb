module Jekyll
    ...
    class CategoryGenerator < Generator
        ...
        def paginate(site, page)
            # sorting based on other categories present in first post of this category list
            # in my site comics all belong to a category called 'comics' and to a category based on the actual name of the comic
            page_category = page.data['category']
            posts = site.categories[page_category]
            other_categories = posts[0].categories
            if other_categories && other_categories.include?('comics')
               category_posts = posts.sort_by { |p| p.date.to_f }
            else
               category_posts = posts.sort_by { |p| -p.date.to_f }
            end
